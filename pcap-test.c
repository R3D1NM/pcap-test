...
{
    u_int8_t ip_hl:4,ip_v:4;         /* length:version */
...
int main(int argc, char* argv[]) {
	...
	while (true) {
		...
		struct libnet_tcp_hdr *tcp_hdr = (struct libnet_tcp_hdr *) (packet+sizeof(*eth_hdr)+ipv4_hdr->ip_hl*4);
		...
		//Payload
		u_char* payload = packet+sizeof(*eth_hdr)+ipv4_hdr->ip_hl*4+tcp_hdr->th_off*4;
		unsigned int len = header->caplen - (sizeof(*eth_hdr)+ipv4_hdr->ip_hl*4+tcp_hdr->th_off*4);
